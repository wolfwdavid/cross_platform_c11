...
- **`lattice init`.** Use **`--workflow classic --preset stage11`** — the orchestrator flow uses canonical status keys (`backlog`, `in_progress`, `review`, `in_validation`, `pr_open`, `done`). The `opinionated` personality's playful display names ("thinking about it", "on it", "shipped") conflict with the architect / orchestrator vocabulary, and the `linear` status preset lacks the `in_validation`/`pr_open` lanes the dispatch loop drives. Pass `--actor`, `--project-code`, `--project-name`, `--model`, plus `--no-setup-claude --no-setup-agents` since the Architect manages those directly.
